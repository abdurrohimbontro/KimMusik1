    ...
    if ban_status["is_banned"]:
        if (
            ...
            await cmd.reply_text(
                f"Maaf, kamu di blokir, tanya di @{GROUP_SUPPORT} jika kamu pikir terjadi kesalahan.",
                quote=True,
            )
            return
...
async def send_msg(user_id, message):
    try:
        ...
        return 400, f"{user_id} : dinonaktifkan\n"
    except UserIsBlocked:
        return 400, f"{user_id} : Memblokir bot\n"
    except PeerIdInvalid:
        return 400, f"{user_id} : id pengguna tidak valid\n"
    except Exception:
